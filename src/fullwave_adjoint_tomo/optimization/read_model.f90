...
! reads in current transverse isotropic model: vpv.. & vsv.. & eta & rho
  ...
  if (ier /= 0) then
    print *,'Error opening: ',trim(m_file)
    print *,'fill eta array with 1.0 ... '
    model_eta(:,:,:,1:nspec) = 1.0_CUSTOM_REAL
    !call exit_mpi(myrank,'file not found')
  else
...
! reads in current transverse isotropic model: vpv.. & vsv.. & eta & rho
  ...
  implicit none
  integer :: ival,ier,IRREGULAR_MESH_N
  ...
  read(IIN) IRREGULAR_MESH_N !BinHe
  ! allocate arrays for storing the databases
  if(allocated(ibool))deallocate(ibool)!BinHe added refering Kai's code
