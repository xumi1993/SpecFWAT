...
! calculates gradient by steepest descent method
  ...
  real(kind=CUSTOM_REAL):: r,max,depth_max
  ! gradient vector norm ( v^T * v )
  real(kind=CUSTOM_REAL) :: norm_bulk,norm_alphav,norm_alphah,norm_betav,norm_betah,norm_eta
  real(kind=CUSTOM_REAL) :: norm_bulk_sum,norm_alphav_sum,norm_alphah_sum,norm_betav_sum, &
  ...
  ! stores model_dbulk, ... arrays
  call write_gradient_tiso()  !!! Kai to be revised !!!
  ...
  if (myrank == 0) then
    print *,'scaled gradient:'
    if (USE_ALPHA_BETA_RHO_TISO) then
    print *,'  betav min/max: ',min_vpv,max_vpv
    print *,'  betah min/max: ',min_vph,max_vph
    else
    print *,'  bulk min/max : ',min_bulk,max_bulk
    endif
    print *,'  betav min/max: ',min_vsv,max_vsv
