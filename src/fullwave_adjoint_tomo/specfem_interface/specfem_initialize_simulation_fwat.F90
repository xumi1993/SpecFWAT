...
! United States and French Government Sponsorship Acknowledged.
  ...
  subroutine initialize_simulation_fwat()
  ...
  integer ier, tmp_NSTEP
  logical BROADCAST_AFTER_READ
  real(kind=CUSTOM_REAL) :: tmp_DT
  ...
  ! DT = tmp_DT
  ! checks flags
  call initialize_simulation_check_fwat()
...
! read the value of NSPEC_AB and NGLOB_AB because we need it to define some array sizes below
  ...
  call dallocate_similation_arrays()
  ...
  call initialize_simulation_adjoint_fwat()

  ! initializes GPU cards
  if (GPU_MODE) call initialize_GPU_fwat()

  end subroutine initialize_simulation_fwat

  subroutine dallocate_similation_arrays()
    use specfem_par
    use specfem_par_elastic
    use specfem_par_acoustic
    use specfem_par_poroelastic
    ...
    if (allocated(Veloc_dsm_boundary)) deallocate(Veloc_dsm_boundary)
    if (allocated(Tract_dsm_boundary)) deallocate(Tract_dsm_boundary)
    if (allocated(Veloc_axisem)) deallocate(Veloc_axisem)
    if (allocated(Tract_axisem)) deallocate(Tract_axisem)
    if (allocated(hxir)) deallocate(hxir)
    if (allocated(hpxir)) deallocate(hpxir)
    if (allocated(hetar)) deallocate(hetar)
    if (allocated(hpetar)) deallocate(hpetar)
    if (allocated(hgammar)) deallocate(hgammar)
    if (allocated(hpgammar)) deallocate(hpgammar)
    if (allocated(div)) deallocate(div)
    if (allocated(curl_x)) deallocate(curl_x)
    if (allocated(curl_y)) deallocate(curl_y)
    if (allocated(curl_z)) deallocate(curl_z)
    
  end subroutine dallocate_similation_arrays
!
!-------------------------------------------------------------------------------------------------
!

  subroutine initialize_simulation_check_fwat()
  ...
  integer :: ier
  real :: seednum
  character(len=MAX_STRING_LEN) :: HEADER_FILE, seedstr
  ...
  if (myrank == 0) then
    call random_seed () 
    call RANDOM_NUMBER(seednum)
    seednum = seednum*1000000
    write(seedstr, '(I06)') int(seednum)
    ...
    open(IOUT,file=trim(OUTPUT_FILES)//'/dummy.'//trim(seedstr)//'.txt', &
    ...
    open(IOUT,file=trim(LOCAL_PATH)//'/dummy.'//trim(seedstr)//'.txt', &
         status='unknown',iostat=ier)
    if (ier /= 0) then
  ...
  end subroutine initialize_simulation_check_fwat

!
!-------------------------------------------------------------------------------------------------
!

  subroutine initialize_simulation_adjoint_fwat()
...
! initialization for ADJOINT simulations
  ...
  end subroutine initialize_simulation_adjoint_fwat

!
!-------------------------------------------------------------------------------------------------
!

  subroutine initialize_GPU_fwat()
...
! initialization for GPU cards
  ...
  end subroutine initialize_GPU_fwat
