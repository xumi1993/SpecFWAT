...
! The region of regular grid should totally contain the SEM domain
  ...
  character(len=MAX_STRING_LEN) :: prname, prname_lp, data_filename, fname, grid_file, model_name
  ...
  if (command_argument_count() ==0) then
    if (myrank == 0) then
      print *,'USAGE:  mpirun -np NPROC bin/xcombine_vol_data_on_regular_grid grid_file &
  ...
  if (myrank == 0) then
    ...
    do i=1,projection_fd%nz
      zfd(i) = (projection_fd%oz + (i-1) * projection_fd%hz)/1000
    enddo
