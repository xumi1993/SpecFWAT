...
set mod = M03
set dir = .
set md = $dir/model.rho.${mod}.regridded.xyz
set md1 = $dir/model.vp.${mod}.regridded.xyz
set md2 = $dir/model.vs.${mod}.regridded.xyz
...
set PS = fig_profile_${mod}_${eid}_${stnmp}.pert.ps
...
#!
#mv svel13.cpt $CPT
makecpt -Csvel13m.cpt -T$cmin/$cmax/2 >$CPT
makecpt -Csvel13m.cpt -T$cmin/$cmax/2 >$CPT1
makecpt -Csvel13m.cpt -T$cmin/$cmax/2 >$CPT2
...
foreach dep  ( `seq 0 2.5 200` )
  echo $dep
  set rad = `echo $dep |awk '{print 6371-$1}'`;
  # M16
  cat image.3D |awk -v a=$rad '{if(sqrt(($3-a)**2)<1) print $1,$2,$4*10**s}' s=$scale >dep.xyd.temp
  set mean = `cat dep.xyd.temp |awk 'BEGIN{sum=0.0;num=0} {{sum=sum+$3;num=num+1}} END{print sum/num}'`
  cat dep.xyd.temp |awk '{print $1,$2,($3-a)/a*100}' a=$mean >dep.xyd
  ...
  # M21
  cat image.3D.1 |awk -v a=$rad '{if(sqrt(($3-a)**2)<1) print $1,$2,$4*10**s}' s=$scale >dep.xyd.temp
  set mean = `cat dep.xyd.temp |awk 'BEGIN{sum=0.0;num=0} {{sum=sum+$3;num=num+1}} END{print sum/num}'`
  cat dep.xyd.temp |awk '{print $1,$2,($3-a)/a*100}' a=$mean >dep.xyd
  #xyz2grd dep.xyd -R$RMAP -I0.02 -Gdep.grd 
  surface dep.xyd -R$RMAP -I0.02 -Gdep.grd 
  ...
  # Barak2015
  cat image.3D.2 |awk -v a=$rad '{if(sqrt(($3-a)**2)<1) print $1,$2,$4*10**s}' s=$scale >dep.xyd.temp
  set mean = `cat dep.xyd.temp |awk 'BEGIN{sum=0.0;num=0} {{sum=sum+$3;num=num+1}} END{print sum/num}'`
  cat dep.xyd.temp |awk '{print $1,$2,($3-a)/a*100}' a=$mean >dep.xyd
  #xyz2grd dep.xyd -R$RMAP -I0.02 -Gdep.grd 
  surface dep.xyd -R$RMAP -I0.02 -Gdep.grd 
...
###
set range1 = `minmax -C $FV | awk ' { print $1 "/" $2 "/" "0/4" }'`
project -C$GC1 -E$GC2 -G2 -Q -Lw >track.dat
grdtrack track.dat -G/home/wangkai/GLOB_TOPO/ETOPO1_Ice_g_gmt4.grd |awk '{print $1,$2,$3,$4}'>surf.topo
psbasemap -JX${xwidth}i/0.4i -R$range1 -B"$xtick"/a2f1:"Topo (km)":We -K -X8 -Y20 -P > $PS
cat surf.topo |awk '{print $3,$4/1000.}' |psxy -J -R -O -K >>$PS
psxy -J -R -SV0.05/0./0. -Gred -O -K >>$PS <<EOF
81.7379 0. 0. 0.4 
EOF
psxy -J -R -SV0.05/0./0. -Gblue -O -K >>$PS <<EOF
113.0618 0. 0. 0.4
222.5263 0. 0. 0.4
318.3576 0. 0. 0.4
EOF
pstext -R -J -Gred -O -K -N -P <<EOF >>$PS
81.7 3 12 0 0 6 SAF 
EOF
pstext -R -J -Gblue -O -K -N -P <<EOF >>$PS
160. 3 12 0 0 6 GV
275. 4 12 0 0 6 SNB
EOF
...
#M16
psbasemap -R$range -J$SCALE -B"$xtick"/"$ytick":"Depth (km)":Wesn -Y-1.6i -K -O >> $PS
...
#psclip -J -R -C -O -K >>$PS
psscale -D3.4i/0.8i/1.3i/0.5 -C$CPT -Ba4f2/:"Rho (%)": -V -E -O -K -N >> $PS
...
$tx1 -70 12 0 2 MC $t1[$i]
$tx2 -70 12 0 2 MC $t2[$i]
EOF
...
#grdcontour slice.grd.3 -W1p,black -C0.05 -L3.0/4.7 -A+g+o+f1+p1+s8+N -S -R -J -O -K >> $PS
#psclip -J -R -C -O -K >>$PS
psscale -D3.4i/0.8i/1.3i/0.5 -C$CPT1 -Ba4f2/:"Vp (%)": -V -E -O -K -N >> $PS
#grdcontour slice.grd -J -R -O -K -Ccont.d -W1/255 >> $PS 
### plot moho
#cat moho.dat |psxy -J -R -W1p -O -K >> $PS
#pstext -R -J -O -K -N -P <<EOF>>$PS
...
#psclip -J -R -C -O -K >>$PS
psscale -D3.4i/0.8i/1.3i/0.5 -C$CPT2 -Ba4f2/:"Vs (%)": -V -E -O -K -N >> $PS
#grdcontour slice.grd -J -R -O -K -Ccont.d -W1/255 >> $PS 
### plot moho
#cat moho.dat |psxy -J -R -W1p -O -K >> $PS
#pstext -R -J -O -K -N -P <<EOF>>$PS
