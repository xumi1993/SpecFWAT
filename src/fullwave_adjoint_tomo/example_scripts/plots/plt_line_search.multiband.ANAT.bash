...
newmod=`echo $mod |awk -FM '{printf"M%02d\n",$2+1}'`
iternum=`echo $mod |awk -FM '{printf"%d\n",$2-15}'`

#for band in T006_T015 T010_T020 T015_T035 T020_T040;do
...
# calculate misfit of step 0.00
  sumf=0.0
  sumn=0
  cat ../src_rec/sources_ls.dat |awk '{printf"%s\n",$1}' >eid.dat
  for eid in `cat eid.dat`;do
    #eid=`echo $line |awk '{printf"%s.%s",$2,$1}'`
    misf=`cat ../misfits/${mod}.set*_${band}*_window_chi |awk '{if($1==a) print $0}' a=$eid  |awk 'BEGIN{sum=0;} {if($29!=0) {sum=sum+$29}} END{print sum}' `
    nmeas=`cat ..//misfits/${mod}.set*_${band}*_window_chi |awk '{if($1==a) print $0}' a=$eid |awk 'BEGIN{n=0;} {if($29!=0) {n=n+1}} END{print n}' `
...
#  for step in 0.010 0.020 0.030 0.040;do ### M18
  for step in `grep STEP_LENS ../fwat_params/FWAT.PAR |awk -F: '{print $2}'`;do  ### M19
    input=chi_${mod}_slen${step}_${band}.dat
    cat /dev/null >$input
    for file in `ls ../misfits/${mod}_step${step}.ls_${band}_*_chi`;do
      evt=`echo $file |awk -F../misfits/ '{print $2}' |awk -F_ '{print $4 }'`
# MTTT dT & sigma
...
#for slen in 0.0 0.010 0.020 0.030 0.040;do ### M18
for slen in `grep STEP_LENS ../fwat_params/FWAT.PAR |awk -F: '{printf"0.00 %s",$2}'` ;do
   sumf=0.0
   sumn=0
   #for band in T006_T015 T010_T020 T015_T035 T020_T040;do
   for band in T006_T015 T010_T020 T015_T035;do
...
cat $mod.T006_T015.mis |awk '{print $1,$4}' |psxy -JX -R -Sc0.25 -Gcyan -O -K >> $out
cat $mod.T010_T020.mis |awk '{print $1,$4}' |psxy -JX -R -Sc0.25 -Gcyan -O -K >> $out
cat $mod.T015_T035.mis |awk '{print $1,$4}' |psxy -JX -R -Sd0.25 -Gblue -O -K >> $out
#cat $mod.T020_T040.mis |awk '{print $1,$4}' |psxy -JX -R -St0.25 -Gred -O -K >> $out
cat $mod.mis.avg  |psxy -JX -R -Sh0.25 -Gpurple -O -K >> $out
cat $mod.mis.avg  |psxy -JX -R  -W1p,black -O -K >> $out
rm $mod.T*T*.mis
### plot legend 
# for M17 M18
pslegend -J -R -D0.0/2.1/7.7/2.4/LT -F -O <<eof >>$out
S 0.3c t 0.15i red black 0.6c 20_40 s band
S 0.3c d 0.15i blue black 0.6c 15_30 s band
S 0.3c c 0.15i cyan black 0.6c 10_20 s band
S 0.3c h 0.15i purple black 0.6c 20_40, 15_30 and 10-20 s band
eof
