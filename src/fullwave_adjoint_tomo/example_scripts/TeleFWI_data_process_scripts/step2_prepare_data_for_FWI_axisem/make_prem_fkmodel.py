...
print("LAYER %d %.3f %.3f %.3f %.3f") %(1,float(data[3])*1000,float(data[1])*1000,float(data[2])*1000,-float(data[0])*1000+5000.) # add 5km topography
f.readline()
# read lines 3-4
lay2=f.readline()
data=lay2.split()
...
f.readline()
# read lines 6-14
data=np.genfromtxt(f, dtype="f8,f8,f8,f8,f8,f8", names=['dep', 'vp', 'vs', 'rho', 'dump', 'dump'],max_rows=8)
f.close()
