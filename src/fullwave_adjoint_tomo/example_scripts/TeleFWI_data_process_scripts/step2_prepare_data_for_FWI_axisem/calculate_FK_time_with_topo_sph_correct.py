...
if len(sys.argv) != 4:
  print("Usage: correct_wavefront_time.py FKmodel FKfile time_pred")
  exit()
else:
  print(str(sys.argv))
...
f.close()
###### read prediected time table file #######
ttp=np.zeros(500)  ### !!!! NEED TO CHANGE !!!
f1=open(predtt,'r')
for i, line in enumerate(f1):
  data=line.split()
  #print(data)
  ttp[i]=float(data[5])  #  ttp: s
  print('i,pred: %d %f\n' %(i,ttp[i]))
f1.close()
nttp=i+1
print('nttp: %d' % nttp)
...
tdelay=0.
is_utm=1
topo_corr=np.zeros(nttp)
sph_corr=np.zeros(nttp)
itt=0
...
utm2latlon = Proj(proj='utm', ellps='WGS84', zone='30')
fp=open(fkfile,'r')
fp1=open('temp.dat','w')
for i, line in enumerate(fp):
  ...
  if i==7:
    ...
    print('nsta:%d' % nsta)
    if nsta!=nttp:
      print('Error: nttp not match nsta!!!')
      sys.exit()
  if i>=15 and i<nsta+15:
    ...
    if kpsv==1:
      ...
      topo_corr[itt]=float(stel)/1000.  ###  !!!!!  Add TOPO correction here !!!  ###
      sph_corr[itt]=ttp[i-15]-tdelay[0]
    elif kpsv==2:
      tdelay=p*(xi-x0)*np.cos(phi_FK/180.*np.pi)+p*(yi-y0)*np.sin(phi_FK/180*np.pi)+eta_s*(0-z0)
    print('%s %s %f %f %f\n' %(netwk,stnm,tdelay[0],topo_corr[itt],sph_corr[itt]))
    fp1.write('%s %s %f %f %f %f \n' %(netwk,stnm,dist/1000.,tdelay[0],topo_corr[itt],sph_corr[itt]))
    itt=itt+1
fp.close()
fp1.close()
#### write
print('Now get relative sph correction term')
mean=sph_corr.mean(axis=0)
print ('mean of sph_corr: %f' %mean)
fp=open('temp.dat','r')
...
for i, line in enumerate(fp): 
   ...
   stnm=data[1]
   dist=float(data[2])
   tdelay1=float(data[3])
   topo_corr1=float(data[4])
   sph_corr1=float(data[5])-mean
   tt_corr=tdelay1+topo_corr1+sph_corr1
   print('%s %s %f %f %f\n' %(netwk,stnm,tdelay1,topo_corr1,sph_corr1))
   fp1.write('%s %s %f %f %f %f %f \n' %(netwk,stnm,dist,tt_corr,tdelay1,topo_corr1,sph_corr1))
fp.close()
fp1.close()
