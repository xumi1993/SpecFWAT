...
while read line;do
  ...
  while read line1;do
    ...
    saco_r=$outdir/$evid/$netwk.$stnm..BHR.sac
    saci_t=$indir/$evid/$netwk.$stnm..BHT.sac
    saco_t=$outdir/$evid/$netwk.$stnm..BHT.sac
    saci_z=$indir/$evid/$netwk.$stnm..BHZ.sac
    saco_z=$outdir/$evid/$netwk.$stnm..BHZ.sac

    ### Shift the seismograms by the propogating time above ###

    tt=`cat FKtimes.dat |sed -n "${ista}p" |awk '{print -$4-a+b}' a=$tmin b=$tm`
    echo $stnm $tm $tt
