...
EOF
  ...
  echo "NLAYER             $nlay"  >>./${osdir}/FKmodel_${ievt}
  python make_${mod}_fkmodel.py >>./$osdir/FKmodel_${ievt}
...
EOF
  ...
  #for comp in R Z;do
    ...
    cp $datadir/$ievt/$stalst $osdir/STATIONS_${ievt} 
  #done
  ...
  cat >$osdir/CMTSOLUTION_${ievt} <<eof
...
eof
  # sources.dat
  echo $ievt $evla $evlo 0.0 0.0 >>$osdir/sources.dat 
  #----------------------------------------------------
  let ievt=ievt+1
done
