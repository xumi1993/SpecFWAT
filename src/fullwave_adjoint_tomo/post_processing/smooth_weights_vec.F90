...
#include "config.fh"
  ...
  subroutine smoothing_weights_vec_glob(x0,y0,z0,sigma_h2,sigma_v2,exp_val, &
...
! in all known applications, and in the main program we check that N_SLS == 3 if FORCE_VECTORIZATION is used and we stop
  ...
  end subroutine smoothing_weights_vec_glob
...
!
  ...
  subroutine get_distance_vec_glob(dist_h,dist_v,x0,y0,z0,x1,y1,z1)
...
! returns vector lengths as distances in radial and horizontal direction
  ...
  end subroutine get_distance_vec_glob
