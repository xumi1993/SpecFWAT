!----------------------------------------------------------------------
  ...
  character(len=240) :: file_obs, file_syn
...
!----------------------------------------------------------------------
  ...
  character(len=120) :: basename
  character(len=240) :: file_obs_env, file_syn_env
  double precision :: max_value_obs, max_value_syn, max_value
...
! -----------------------------------------------------------------------
  ...
  call sacio_newhead(head, sngl(dt), npts, sngl(b))
  head%kstnm = trim(kstnm)
  head%knetwk = trim(knetwk)
  head%kcmpnm = trim(kcmpnm)
  head%b = sngl(b)
  head%delta = sngl(dt)
  head%npts = npts
  head%iftype = 1
  ...
  call sacio_writesac(file_obs, head, obs_lp(1:npts), io_error)
  call sacio_writesac(file_syn, head, synt_lp(1:npts), io_error)
...
! -----------------------------------------------------------------------
  ...
  subroutine write_mt_input_2(basename,file_o,file_s)
    ...
    character(len=120) :: basename
    character(len=240) :: file_o, file_s
    character(len=240) :: file_mt
