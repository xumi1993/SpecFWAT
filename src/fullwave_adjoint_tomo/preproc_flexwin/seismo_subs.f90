...
!----------------------------------------------------------------------
  ...
  double precision :: b ! Kai revised for FWAT
  character(len=8) :: kstnm,knetwk,kcmpnm
  integer :: npts
  double precision :: evla, evlo, stla, stlo, evdp
  double precision :: azimuth, backazimuth, dist_deg, dist_km
  real :: P_pick, S_pick
  ...
  integer :: num_phases
  character(len=8), dimension(MAX_PHASES) :: ph_names
...
!----------------------------------------------------------------------
  ...
  subroutine read_parameter_file_flexwin()
  ...
  open(unit=IIN,file='fwat_params/FLEXWIN.PAR',status='old')
...
!----------------------------------------------------------------------
  ...
  character (len=*) :: file_s, file_o
  integer :: ier
  ! local parameters
  double precision :: b1, dt1, b2, dt2 , synt_sngl(NDIM), obs_sngl(NDIM)
...
! read synthetic
  ...
  call sacio_readsac(file_s, head, data_read, nerr)
  b1 = dble(head%b)
  dt1 = dble(head%delta)
  npts1 = head%npts
  synt_sngl(1:npts1) = data_read
  ...
  synt(:) = 0.
  synt(1:npts1) = synt_sngl(1:npts1)

! read observed
  if (allocated(data_read)) deallocate(data_read)
  call sacio_readsac(file_o, head, data_read, nerr)
  b2 = dble(head%b)
  dt2 = dble(head%delta)
  npts2 = head%npts
  obs_sngl(1:npts2) = data_read
  if (nerr.ne.0) then
    write(*,*)
...
!----------------------------------------------------------------------
  ...
  subroutine bandpass_flexwin(x, n, delta_t, x_filt)
  use seismo_variables
  implicit none
...
!  delta_t_sngl = sngl(delta_t)
  ...
  call xapiir(x_sngl,n,'BU',sngl(TRBDNDW),sngl(APARM),IORD,'BP',sngl(FSTART),sngl(FEND),sngl(delta_t),PASSES)

  x_filt(1:n) = dble(x_sngl(1:n))

  deallocate(x_sngl)

  end subroutine bandpass_flexwin
...
!----------------------------------------------------------------------
  ...
  obs_lp(:) = 0.
  synt_lp(:) = 0.
  call bandpass_flexwin(obs,npts,dt,obs_lp)
  call bandpass_flexwin(synt,npts,dt,synt_lp)
...
!------------------------------------------------------------------------
  ...
  integer, intent(in) :: i
  character(len=8), intent(out) :: istring
...
! get the travel times
  ...
  call ttimes(real(dist_deg),real(evdp),num_phases,ph_names,ph_times)
