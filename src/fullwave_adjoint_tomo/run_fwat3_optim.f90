subroutine run_optim()
  ...
  real(kind=CUSTOM_REAL), dimension(:,:,:), allocatable           :: model_tmp
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable         :: model_all
  type(hdf5_file) :: h5fp
...
! optimize (SD, CG or LBFGS)
  ...
  if (tomo_par%DO_LS) then
    ...
    do sit = 1, tomo_par%MAX_SUB_ITERS
      ...
      ! grid model to regular grid
      if (is_joint) then
        call get_kernel_names()
        model_all = zeros(kernel_num, rg%nx, rg%ny, rg%nz)
        GRID_PATH = 'optimize/model_ls.h5'
        if (myrank == 0) call h5fp%open(GRID_PATH, status='replace')
        do i = 1, kernel_num
          call rg%gridmodel(trim(LOCAL_PATH), trim(fwat_kernel_names(i)), model_tmp)
          if (myrank == 0) call h5fp%add('/'//trim(fwat_kernel_names(i)), model_tmp)
        enddo
        if (myrank == 0) call h5fp%close()
        call synchronize_all()
      endif
