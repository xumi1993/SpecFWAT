module projection_on_FD_grid_fwat
...
subroutine nearest_interp(x_sem, y_sem, z_sem, model_on_FD_grid, value)
  real(kind=CUSTOM_REAL)                                 :: x_sem, y_sem, z_sem, min_dist, &
                                                             xfd, yfd, zfd, dist
  real(kind=CUSTOM_REAL), intent(out)                    :: value
...
!--------------------------------------------------------------------------------------------------------------------
subroutine read_fd_grid_parameters_for_projection(fname)

!   use shared_input_parameters, only: SUPPRESS_UTM_PROJECTION
  integer ier
  character(len=MAX_STRING_LEN) :: fname

  open(676,file=fname,iostat=ier)
...
!!$                endif
    ...
    if (DEBUG_MODE) write(*,*) ' END  compute_interpolation_coeff_FD_SEM step 1',  nb_fd_point_loc
    ...
    allocate(projection_fd%heta(NGLLY,nb_fd_point_loc),stat=ier)
    if (ier /= 0) call exit_MPI_without_rank('error allocating array 151')
    allocate(projection_fd%hgamma(NGLLZ,nb_fd_point_loc),stat=ier)
...
!!$    enddo
    ...
    if (DEBUG_MODE) write(*,*) ' END  compute_interpolation_coeff_FD_SEM step 2', nb_fd_point_loc
...
subroutine find_nearestZ_nonzero(data, i, j, k, nx, ny, nz)
  implicit none
  integer :: nx, ny, nz, i, j, k, k0, dist, min_dist
  real, dimension(nx, ny, nz) :: data
  
  min_dist = 100000
  do k0 = max(k-1, 1), min(k+1, nz)
    dist = abs(k0 - k)
    if (dist < min_dist .and. data(i, j, k0) /= 0) then
      min_dist = dist
      data(i, j, k) = data(i, j, k0)
    endif
  enddo

end subroutine find_nearestZ_nonzero

subroutine find_nearestXY_nonzero(data, i, j, k, nx, ny, nz)
  implicit none
  integer :: nx, ny, nz, i, j, k, i0, j0, dist, min_dist
  real, dimension(nx, ny, nz) :: data
  
  min_dist = 100000
  do i0 = max(i-1, 1), min(i+1, nx)
    do j0 = max(j-1, 1), min(j+1, ny)
      dist = abs(i0 - i) + abs(j0 - j)
      if (dist < min_dist .and. data(i0, j0, k) /= 0) then
        min_dist = dist
        data(i, j, k) = data(i0, j0, k)
      endif
    enddo
  enddo
end subroutine find_nearestXY_nonzero
