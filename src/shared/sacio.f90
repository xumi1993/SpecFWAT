...
!       2022-04-09  Mijian Xu @ NTU double precision to read and write sac
!
!=======================================================================
module sacio
...
if (.not. allocated(real_data)) allocate(real_data(1:head%npts), stat=flag)
if (.not. allocated(real_data)) allocate(data(1:head%npts), stat=flag)
real_data(:) = 0
if (flag /= 0)then
    flag = 3
...
implicit none
character(len=*), intent(in) :: filename
real, dimension(:), allocatable :: real_data
...
if (.not. allocated(real_data)) allocate(real_data(head%npts))
real_data(:) = real(data)
write(50, iostat=flag) real_data
if (flag /= 0) then
   flag = 5
   write(*, *) "Error in writing SAC file ", filename
   close(unit=50)
