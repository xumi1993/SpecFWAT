...
module utils
  ...
  interface transpose_3
    module procedure transpose_3_s, transpose_3_f
  end interface transpose_3
...
!=======================================================================
  ...
  pure subroutine append_ch_name(list, value)
    character(len = MAX_STRING_LEN), dimension(:), allocatable, intent(inout) :: list
    character(len = MAX_STRING_LEN), intent(in) :: value
    character(len = MAX_STRING_LEN), dimension(:), allocatable :: tmp
  ...
  subroutine gradient_2_geo(f, lon, lat, tx, ty)
    ...
    real(kind=DPRE), dimension(:,:), intent(in) :: f
    real(kind=DPRE), dimension(:), intent(in) :: lon, lat
    real(kind=DPRE), dimension(:,:), allocatable, intent(out) :: tx, ty
    real(kind=DPRE), dimension(:,:), allocatable :: dx, dy
    real(kind=DPRE), dimension(:), allocatable :: dx1, dy1
    integer :: nx, ny, ix, iy
    real(kind=DPRE), dimension(:,:), allocatable :: gfx, gbx, gfy, gby
    ...
    dx = R_EARTH_KM*dx*deg2rad
    dy = R_EARTH_KM*dy*deg2rad
  ...
  subroutine gradient_2(f, dx, dy, tx, ty)
    real(kind=DPRE), dimension(:,:), allocatable, intent(in) :: f
    real(kind=DPRE), intent(in) :: dx, dy
    real(kind=DPRE), dimension(:,:), allocatable, intent(out) :: tx, ty
    real(kind=DPRE), dimension(:,:), allocatable :: gfx, gbx, gfy,gby
    integer :: nx, ny, ix, iy
  ...
  pure function gaussian_smooth_geo_2(data, lons, lats, sigma) result(smdata)
    real(kind=DPRE), intent(in) :: sigma ! in deg
    real(kind=DPRE), dimension(:), intent(in) :: lons, lats
    real(kind=DPRE), dimension(:,:), intent(in) :: data
    real(kind=DPRE), dimension(:,:), allocatable :: smdata
    real(kind=DPRE), dimension(:,:), allocatable :: delta, w, lala, lolo
    integer :: i, j, n1, m1, n2, m2, dims(2)
    real(kind=DPRE) :: sigma3, wsum, dx, dy, sigma_sq
    integer :: nx, ny
  ...
  pure function smooth_1(data, z, sigma) result(smdata)
    real(kind=DPRE), intent(in) :: sigma
    real(kind=DPRE), dimension(:), intent(in) :: data, z
    real(kind=DPRE), dimension(:), allocatable :: smdata
    real(kind=DPRE), dimension(:), allocatable :: w
    integer :: i, n1, n2
    real(kind=DPRE) :: sigma3, wsum, sigma_sq, dz
    integer :: nx
    ...
    do i = 1, size(data)
      ...
      w = exp(-((arange(n1,n2) - i)*dz)**2 / (2.0 * sigma_sq))
  ...
  pure function gps2dist_scalar(lat0, lon0, lat1, lon1) result(dist)
    ! Input parameters
    real(kind=DPRE), intent(in) :: lon0, lon1, lat0, lat1
    ! Local variables
    real(kind=DPRE) :: dlat, dlon, a, deg,dist
  ...
  pure function gps2dist_1(lat0, lon0, lat1, lon1) result(dist)
    ! Input parameters
    real(kind=DPRE), intent(in) :: lon0, lat0
    real(kind=DPRE), dimension(:), intent(in) :: lon1, lat1
    real(kind=DPRE), dimension(:), allocatable :: dist
    ! Local variables
    real(kind=DPRE), dimension(:), allocatable :: dlat, dlon, a, deg
