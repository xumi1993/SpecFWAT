module ma_sub
  ...
  use ascii_rw
  use sacio
  ! use interpolation_mod, only: bwfilt
...
contains
  ...
  ! ====================================================================
    ...
    complex(kind=8), dimension(:), intent(out) :: trans_w
    ...
    complex(kind=8), dimension(NPT) :: syn_dtwo, dat_dtwo, syn_dtw_ho, dat_dtw_ho,  &
    ...
    call fft_ma(LNPT,syn_dtwo,FORWARD_FFT,dt)
    call fft_ma(LNPT,dat_dtwo,FORWARD_FFT,dt)
    ...
    do ictaper = 1, ntaper
      ...
      call fft_ma(LNPT,syn_dtw_ho,FORWARD_FFT,dt)
      call fft_ma(LNPT,dat_dtw_ho,FORWARD_FFT,dt)
  ...
  subroutine mt_adj(istart,dat_dtw,syn_dtw,syn_dtw_phydisp,nlen,dt,tshift,dlnA,sigma_dt_cc,sigma_dlnA_cc, &
    ...
    complex(kind=8), dimension(NPT) :: d_bot_mtm, v_bot_mtm
    ...
    complex(kind=8), dimension(:,:),allocatable :: syn_dtw_ho_all, syn_vtw_ho_all
    complex(kind=8), dimension(NPT) :: pwc_adj,qwc_adj
...
!!$    close(88)
      ...
      do ictaper = 1,ntaper
        ...
        call fft_ma(LNPT,syn_dtw_ho_all(:,ictaper),FORWARD_FFT,DT)
        call fft_ma(LNPT,syn_vtw_ho_all(:,ictaper),FORWARD_FFT,DT)
      ...
      do ictaper = 1,ntaper
        ...
        call fftinv_ma(LNPT,pwc_adj,REVERSE_FFT,dt,dtau_pj_t)
        call fftinv_ma(LNPT,qwc_adj,REVERSE_FFT,dt,dlnA_qj_t)
  ...
  subroutine drsac1(datafile,data,npt1,b1,dt1)
    ...
    double precision, dimension(NDIM_MA), intent(out) :: data
    double precision, dimension(:), allocatable :: data_read
    integer :: npt1, flag
    double precision :: b1,dt1

    ! read file as double precision
    if (allocated(data_read)) deallocate(data_read)
    call sacio_readsac(datafile, head, data_read, flag)
    ! return double precision quantities
    b1 = dble(head%b)
    dt1 = dble(head%delta)
    npt1 = head%npts
    data(1:npt1) = data_read
  ...
  subroutine dwsac1(datafile,data,npt1,b1,dt1)
    ...
    call sacio_newhead(head, sngl(dt1), npt1, sngl(b1))
    ...
    call sacio_writesac(datafile, head, data, flag)
  end subroutine dwsac1

  !-----------------------------------------------------------------------------

  subroutine dwsac0(datafile,data,npt1,b1,dt1,ntw,stnm,cmpnm,dist,az,baz,stla,stlo,evla,evlo,evdp)
    ! convert to single precision, then write sac file
    ! --> Kai Wang 05/2020, added more headers: ntw,stnm,cmpnm,dist,az,baz,stla,stlo,evla,evlo
    ! --> includes an option to add minmax values to sac file,
    !     which are used in the plotting scripts
    ...
    double precision, dimension(npt1), intent(in) :: data
    double precision, intent(in) :: b1,dt1
    character(len=*),intent(in) :: ntw,stnm,cmpnm
    double precision, intent(in) :: dist,az,baz,stla,stlo,evla,evlo,evdp
    logical, parameter :: minmax_header = .true.
    integer :: flag
    ...
    if (minmax_header) then
      ! set minmax values in sac file
      dat_sngl = real(data)
      head%depmin = minval(dat_sngl)
      head%depmax = maxval(dat_sngl)
      head%knetwk = trim(ntw)
      head%kstnm = trim(stnm)
      head%kcmpnm = trim(cmpnm)
    endif
    call sacio_writesac(datafile, head, data, flag)
  end subroutine dwsac0

  !Mijian add for saving RF. user0=rayp, user1=f0
  subroutine dwsac_rf(datafile,data,npt1,b1,dt1,ntw,stnm,cmpnm,dist,az,baz,stla,stlo,rayp,f0)
    ...
    double precision, intent(in) :: b1,dt1
    character(len=*),intent(in) :: ntw,stnm,cmpnm
    double precision, intent(in) :: dist,az,baz,stla,stlo
    real, intent(in) :: rayp,f0
    logical, parameter :: minmax_header = .true.
    integer :: flag
    real, dimension(npt1) :: dat_sngl
...
!!$    close(88)
  ...
  subroutine write_trans(filename, trans, wvec, fnum, i_right, idf_new, df, tshift, dlnA, &
    ...
    implicit none
    character(len=*), intent(in) :: filename
    complex(kind=8), intent(in) :: trans(:)
...
!!$    endif
  ...
  subroutine reconstruct_syn(filename, syn_dtwo, wvec, dtau_wt, dlnA_wt, &
    ...
    implicit none
    character(len=*), intent(in) :: filename
    complex(kind=8), dimension(:), intent(in) ::  syn_dtwo
    ...
    complex(kind=8), dimension(NPT) :: wseis_recon
    integer i
    ...
    enddo
    call fftinv_ma(LNPT,wseis_recon,REVERSE_FFT,dt,syn_dtw_mt)
    ...
    enddo
    call fftinv_ma(LNPT,wseis_recon,REVERSE_FFT,dt,syn_dtw_mt_dt)
...
!-------------------------------------------------------------------
   ...
   subroutine interpolate_syn(syn,t1,dt1,npt1,t2,dt2,npt2)
     ...
     double precision :: syn1(NDIM_MA), time, tt
     integer i, ii
...
!-------------------------------------------------------------------
   ...
   subroutine read_par_file(fstart0,fend0,tt,dtt,nn,chan)
     ...
     open(10,file='./fwat_params/MEASUREMENT.PAR',status='old',iostat=ios)
...
!-------------------------------------------------------------------
  ...
  subroutine get_sacfile_header(data_file,yr,jda,ho,mi,sec,ntw,sta, &
    ...
    call sacio_readhead(data_file, head, nerr)
    if(nerr /= 0) then
      write(*,*)'Error reading SAC header: ', trim(data_file)
      call exit(-1)
    endif
...
!  use ma_sub,only: get_sacfile_header,drsac1
  ...
  double precision, dimension(NDIM_MA) :: data, syn
