...
c
      if(debug) write (*,*) 'DEBUG - calling rough'
 ...
 202  format(/1x,'delx =',2f10.2/1x,'dpmax -',2f12.6/1x,'drmax =',
     1 2f10.2)
      ...
      xtol=xtol/a0
c
      if(debug) write (*,*) 'DEBUG - calling findcp'
      call findcp(ncr)
...
c     call retrns(11)
c
      if(debug) write (*,*) 'DEBUG Calling pgrid with', 1, n1, xtol
...
c205  format(1x,i5,3f12.6,3f10.2)
      if(debug) write (*,*) 'DEBUG Calling pgrid with', 2, n2, xtol
...
c    1 a0*(xx(i,2)-xx(i-1,2)),rr(i,2),rr(i-1,2)-rr(i,2),i=2,n2)
      if(debug) write (*,*) 'DEBUG Done calling pgrid with', 2, n2, xtol
c
      n=0
      j1=1
      k1=1
...
c   Get the radii of model discontinuities.
      ...
      if(debug) write(*,*) 'DEBUG',np,rd,modnam
