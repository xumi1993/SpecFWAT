...
!
  ...
  if (myrank == 0) then
    ...
    block
      real :: seednum
      character(len=6) :: seedstr
      call random_seed () 
      call RANDOM_NUMBER(seednum)
      seednum = seednum*1000000
      write(seedstr, '(I06)') int(seednum)
      open(IOUT,file=trim(OUTPUT_FILES)//'/dummy.'//trim(seedstr)//'.txt',status='unknown',iostat=ier)
      if (ier /= 0) then
      ...
      open(IOUT,file=trim(LOCAL_PATH)//'/dummy.'//trim(seedstr)//'.txt',status='unknown',iostat=ier)
      if (ier /= 0) then
